% This code is MLPMCMC for the kuramoto model 
close all;
clear;
clc;
format long
...
kuramoto_obs = readmatrix('kuramoto_obs_T_100.txt');
T = 100;
Lmin = 4;
LP = 5;
sigma_obs = 1;
...
N_count_1 = 0;
N_count_last_1 = 0;
Sigma_A1 = 0.1*diag([1,3.5,1.6]);
Sigma_A = 0.12*diag([1.5,1.5,1]);
%Sigma_A = 0.02*diag([1.5,1.5,1]);
for iter = 1:Nl(1)
...
function lpos_p = l_posterior(Theta, lik)
    log_lik = lik;
    log_prior = lG(Theta(1),0,0.1) + lG(Theta(2), -1, 0.6) +  lG(Theta(2), -0.25, 0.2);
