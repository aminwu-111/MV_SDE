...
% discretization size
x0 = 1;
X0_m = cell(LP - Lmin +1, 1);
hl = zeros(LP - Lmin +1,1);
Nl = zeros(LP - Lmin +1,1);
ML = zeros(LP - Lmin +1,1);
for l = 1:LP - Lmin+1
    hl(l) = 2^(-l-Lmin+1);
    Nl(l) =  floor(0.4*2^(2*LP - (l+Lmin-1)) +35);
	ML(l) = floor(0.3 * 2^(2*LP - (l+Lmin-1)) +35);
...
X_measure = simulate_discrete_modkuramoto_p(delta, T, X0_m{1,1}, Theta_SIG_p(1), Theta_SIG_p(2));
Z =  particle_filter(kuramoto_obs, X_measure, delta, T, X0_pf, Theta_SIG_p(1), Theta_SIG_p(2), Theta_SIG_p(3));
lZ = Z;
...
for iter = 1:Nl(1)
    ...
    X_measure_prime = simulate_discrete_modkuramoto_p(delta, T, X0_m{1,1}, Theta_SIG_prime(1), Theta_SIG_prime(2));
    Z_prime =  particle_filter(kuramoto_obs, X_measure_prime, delta, T, X0_pf, Theta_SIG_prime(1), Theta_SIG_prime(2), Theta_SIG_prime(3));
    lZ_prime = Z_prime;
    ...
    if U < alpha_U
        ...
        lZ = particle_filter(kuramoto_obs, X_measure, delta, T, X0_m{1,1}, Theta_SIG_p(1), Theta_SIG_p(2), Theta_SIG_p(3));
...
for l = 1:LP - Lmin 
	...
	[X_1_measure, X_2_measure] = simulate_coupled_discrete_modkuramoto(delta_t, T, X0_m{l+1}, X0_m{l+1},Theta_SIG_l(1), Theta_SIG_l(2));
    ...
    for iter = 1:Nl(l+1)
        ...
        [X_1_measure_p, X_2_measure_p] = simulate_coupled_discrete_modkuramoto(delta_t, T, X0_m{l+1}, X0_m{l+1}, Theta_l_SIG_prime(1), Theta_l_SIG_prime(2));
